...
| **Status** | Implemented on branch `HR-80-implement-endpoints-for-bookings-deliveries-and-returns`, not yet merged to `develop`. `returnNotes` (HR-100) implemented on top of that. `siteAddress` postal-code validation on `PUT /api/bookings/{id}` (HR-116) implemented on branch `HR-116-site-address-postal-code-validation`, also on top of `develop`, also not yet merged. |
