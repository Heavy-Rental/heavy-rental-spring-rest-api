...
One remaining blocker, outside this spec's own code, already flagged in §10:
...
**CORS is no longer a blocker (§10 item 7, fixed 2026-08-11)** — but only for the origins listed in `app.cors.allowed-origins`/`APP_CORS_ALLOWED_ORIGINS`, which defaults to local Vite dev/preview only. Whoever deploys this needs to set that env var to the real frontend origin(s) before a deployed browser build can call this API at all.
