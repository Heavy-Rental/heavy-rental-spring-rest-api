> **Provenance note (added 2026-08-13, this branch — `HR-129-update-api-contracts-2`):** this file is copied verbatim from `specification/SPEC-admin-users-api.md` as it exists on `origin/36-link-rest-api-users-to-front-end` (commit `e79742d`), a **separate, unmerged remote branch**. It is brought into this working tree solely to close a documentation gap flagged by a web-portal API audit: `/api/users` had no entry anywhere in `SPEC-api-index.md` (see that file's §2.7). **The code this spec describes — `UserController`, `UserAdminService`, the `User*` DTOs, the `SecurityConfig` matcher — does not exist in this working tree.** `git show origin/36-link-rest-api-users-to-front-end:src/main/java/com/heavy_rental/rest_api/controller/UserController.java` confirms it exists on that branch; `find src/main/java -iname 'UserController.java'` on this branch finds nothing. Everything below this note is the original file, unedited, including its own "Status" line, which describes its state *on that other branch*, not this one.
