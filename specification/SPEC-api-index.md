...
| **Related specs** | [`SPEC-request-bearer-token.md`](./SPEC-request-bearer-token.md), [`SPEC-auth-login-logout.md`](./SPEC-auth-login-logout.md), [`SPEC-equipment-browse-api.md`](./SPEC-equipment-browse-api.md), [`SPEC-entity-repository.md`](./SPEC-entity-repository.md), [`SPEC-stripe.md`](./SPEC-stripe.md), [`SPEC-haystack-recommender-client.md`](./SPEC-haystack-recommender-client.md), [`SPEC-rental-plan-quote.md`](./SPEC-rental-plan-quote.md), [`SPEC-admin-users-api.md`](./SPEC-admin-users-api.md) (§2.7 — merged, see below), [`SPEC-pricing-estimate.md`](./SPEC-pricing-estimate.md) (§2.5.2 — design only, not built), [`SPEC-spring-proxy-endpoints.md`](./SPEC-spring-proxy-endpoints.md) (every route that does, or deliberately doesn't, call `haystack-fast-api`), [`CHANGES-admin-asset-records.md`](./CHANGES-admin-asset-records.md) (§2.3 — `/api/equipment` → `/api/assets` rename + admin-gating, see below) |
...
|---|---|---|---|---|
| `GET` | `/api/assets` | `ROLE_USER`, `ROLE_ADMIN` | ✅ Merged → `develop` | [`SPEC-equipment-browse-api.md`](./SPEC-equipment-browse-api.md) §7.1 |
| `GET` | `/api/assets/{id}` | `ROLE_USER`, `ROLE_ADMIN` | ✅ Merged → `develop` | §7.2 |
| `POST` | `/api/assets` | `ROLE_ADMIN` only | ✅ Merged → `develop` | §7.3 |
| `PUT` | `/api/assets/{id}` | `ROLE_ADMIN` only | ✅ Merged → `develop` | §7.4 |
| `PATCH` | `/api/assets/{id}` | `ROLE_ADMIN` only | ✅ Merged → `develop` | §7.4 |
| `DELETE` | `/api/assets/{id}` | `ROLE_ADMIN` only | ✅ Merged → `develop` | §7.5 |
| `PUT` | `/api/assets/{id}/image` | `ROLE_ADMIN` only | ✅ Merged → `develop` | §7.6 |
| `GET` | `/api/depots` | `ROLE_USER`, `ROLE_ADMIN` | 🧱 Stub, merged → `develop` | None — always returns `[]`; no `Depot` entity exists (delivery site fields live on `Booking`/`RentalPlan` directly). Comment in `DepotController.java` notes this exists specifically so the React portal's `CustomerPortal` (which errors its whole equipment page if either `/api/assets` or `/api/depots` fails) doesn't break |
...
**Admin-only write gating added 2026-08-13 (closes the gap previously noted here).** The route family was also renamed from `/api/equipment` to `/api/assets` in the same change — `EquipmentController`/`EquipmentRequest`/`EquipmentResponse` are now `AssetController`/`AssetRequest`/`AssetResponse`, matching the `Asset`/`AssetService`/`AssetRepository` naming already used underneath. `SecurityConfig` now gates `POST`/`PUT`/`PATCH`/`DELETE /api/assets/**` to `ROLE_ADMIN` only; `GET` stays on the blanket `ROLE_USER`/`ROLE_ADMIN` rule (see §4) since it also serves the public customer-facing browse feature. New `PUT /api/assets/{id}/image` endpoint (admin-only) lets an admin persist a photo as an `AssetImage` row — previously images existed only via seed data, with no write path at all. The response DTO also now returns `serialno` and `lastConditionUpdatedAt` (both previously present on the `Asset` entity but never surfaced), with `lastConditionUpdatedAt` auto-stamped server-side whenever `condition` actually changes. Full narrative: [`CHANGES-admin-asset-records.md`](./CHANGES-admin-asset-records.md). **Frontend impact:** the React portal's `equipmentApi` currently calls `/api/equipment` — it will need to move to `/api/assets` in a follow-up frontend change; not done as part of this backend-only pass.
...
| 2.5.0 | 2026-08-13 | **Admin asset records: `/api/equipment` renamed to `/api/assets`, writes locked to `ROLE_ADMIN`, new image endpoint.** §2.3 table and surrounding prose updated: `POST`/`PUT`/`PATCH`/`DELETE /api/assets/**` now `ROLE_ADMIN` only (closes the gap this section previously flagged — any authenticated user could write equipment before this change); `GET` unchanged (still open to any authenticated user). `EquipmentController`/`EquipmentRequest`/`EquipmentResponse` renamed to `AssetController`/`AssetRequest`/`AssetResponse` to match the underlying `Asset`/`AssetService` naming. New `PUT /api/assets/{id}/image` (admin-only) persists an uploaded photo as an `AssetImage` row. Response now includes `serialno`/`lastConditionUpdatedAt` (previously on the entity but never returned); `lastConditionUpdatedAt` auto-stamps on real condition changes only. Full narrative in new [`CHANGES-admin-asset-records.md`](./CHANGES-admin-asset-records.md); full contract update in [`SPEC-equipment-browse-api.md`](./SPEC-equipment-browse-api.md). Frontend (`/api/equipment` caller) not yet updated — out of scope for this backend-only change. |
