...
**`rentalPlanId` present → checkout-from-cart path (HR-129).** `items`/`startDate`/`endDate` are ignored (not merged, not validated against) when `rentalPlanId` is set — see `SPEC-rental-plan-quote.md` REQ-6 for the full contract (ownership/status/24-hour-freshness checks, `quote_not_ready`/`quote_expired` error codes, items/dates/price derived from the `RentalPlan`'s own records, plan `status → CONVERTED`). The step-by-step server-side behavior below (steps 1-7) describes the *direct-booking* path only — i.e. `rentalPlanId` absent.
