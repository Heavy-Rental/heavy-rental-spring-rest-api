...
| **Status** | As-built across `develop` (which now includes former `HR-72`/`HR-80` work — see §3.1 — and, as of PR #37/commit `13ec76c`, `36-link-rest-api-users-to-front-end` — see §2.7) + `hr-27-payment-checkout` (§2.4, local/unpushed) + `hr-40-equipment-utilization-tracker` (§2.4, this branch, now merged with `develop`) |
...
`hr-40-equipment-utilization-tracker` is this branch, per the header table, now merged into `develop`. It adds one admin-only route, backing the React portal's admin Overview screen:

| Method | Path | Roles allowed | Status | Contract |
|---|---|---|---|---|
| `GET` | `/api/monthly-utilization` | `ROLE_ADMIN` only | ✅ Merged → `develop` | None — see [`CHANGES-monthly-utilization.md`](./CHANGES-monthly-utilization.md) |

Returns the trailing 6 calendar months' `{id, month, utilization, revenue}` (`MonthlyUtilizationService.getTrailingSixMonths()`). `revenue` sums successful `Payment`s per month (as of the `hr-40` merge, normalized to 2 decimal places via `setScale(2, RoundingMode.HALF_UP)` so zero-payment months serialize as `0.00` rather than `0`); `utilization` sums per-`BookingItem` day-overlap against active-status bookings (`CONFIRMED`/`MOBILISED`/`COMPLETED`) as a percentage of (asset count × days in month) — the denominator is always the full month length, including the in-progress current month, a deliberate choice (not a bug) confirmed in `CHANGES-monthly-utilization.md` §3. Gated by `SecurityConfig`'s `.requestMatchers("/api/monthly-utilization").hasAuthority("ROLE_ADMIN")` — the one route in this index that isn't reachable by a plain `ROLE_USER` token. Verified against a running instance (§2 of the linked change log): `401` with no token, `403` for `ROLE_USER`, `200` for `ROLE_ADMIN`, and cross-checked against seed data end to end through the real portal (not the mock server).
...
| 2.5.0 | 2026-08-13 | **Merged `develop` into `hr-40-equipment-utilization-tracker`** (bringing in 2.1.0–2.4.0 above, including the real `/api/users`/`/api/rentalPlans`/recommender work, since `hr-40` had diverged before any of it existed). Resolved conflicts in this file, `SPEC-entity-repository.md`, `SecurityConfig.java`, `MonthlyUtilizationService.java`, and `CHANGES-monthly-utilization.md`. §2.4's `GET /api/monthly-utilization` row updated from "local to this branch" to `✅ Merged → develop`, and its description updated to note the `revenue` field's `setScale(2, RoundingMode.HALF_UP)` fix (zero-payment months now serialize as `0.00`) and the current-month-denominator behavior confirmed intentional, both decided the same session as this merge. Header `Status` updated to reflect `hr-40` now carrying `develop`'s full route set. |
