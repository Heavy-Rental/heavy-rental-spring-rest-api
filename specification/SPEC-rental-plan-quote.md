...
**THEN** `totalAmount` is set to the sum of all line items' subtotals, `status` moves to `QUOTED`, and `updatedAt` is refreshed to now (**HR-129** — this timestamp is what REQ-6's 24-hour validity window is computed from).
...
- [ ] **HR-129:** Adding/removing an item on a `QUOTED` plan succeeds (not `409`), reverting `status → DRAFT`, clearing `totalAmount`, refreshing `updatedAt` — not yet re-verified live.
- [ ] **HR-129:** `POST /api/bookings` with a `rentalPlanId` — happy path converts the plan (`status → CONVERTED`), booking `totalAmount` matches the plan's quoted total exactly; `404` for a plan not owned by the caller; `409 quote_not_ready` for a non-`QUOTED` plan; `409 quote_expired` for a `QUOTED` plan older than 24 hours; a converted customer can immediately start a new plan (BR-06 no longer blocked). Not yet re-verified live — see §6.2 below for the walkthrough this checklist item is closed by.
