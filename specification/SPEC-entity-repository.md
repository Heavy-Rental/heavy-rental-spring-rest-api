...
`siteAddress` is still nullable at the database/entity level — see the HR-116 note under §5.7, which applies identically here.

**`createdAt`/`updatedAt` now actually written (HR-129).** Both columns existed since this entity's original design but were never set by application code — `RentalPlanService.create()` never called `setCreatedAt`, and nothing ever called `setUpdatedAt` at all, so both persisted `null` on every row. As of HR-129: `create()` sets `createdAt` to the plan's real creation time; `requestQuote()` sets `updatedAt` each time a plan is (re-)quoted, repurposing it as a "last quoted at" stamp rather than adding a new `quotedAt` column — this is what `BookingService`'s 24-hour quote-validity check (`SPEC-rental-plan-quote.md` REQ-6) reads. Item add/remove on an already-`QUOTED` plan also refreshes `updatedAt` when it reverts the plan to `DRAFT` (REQ-2/REQ-3). Both fields are now exposed on `RentalPlanResponse` (`GET`/`POST /api/rentalPlans*`), serialized as ISO-8601 strings (Spring Boot's default `LocalDateTime` serialization, no override in this codebase).
...
| `returnNotes` | `return_notes` | `String(500)` | nullable — added HR-100, mirrors `deliveryNotes`; set only by `ReturnService.updateStatus` on a successful `MOBILISED → COMPLETED` transition (`SPEC-booking-delivery-return-api.md` §5.2/Requirement 6) |
| `createdAt` | `created_at` | `LocalDateTime` | nullable |

**`siteAddress` format (HR-116).** The column itself is still plain nullable `String` at both the entity and database level — no `@Column` length/format constraint was added, and this table's `sitePostalCode` `@Formula` (`substring(site_address from length(site_address) - 5 for 6)`, above) is unchanged. What's new is a DTO-layer guarantee on the three write paths that go through validation: `CreateBookingRequest`/`BookingUpdateRequest` (this entity) and `RentalPlanCreateRequest` (`RentalPlan`, §5.5) now reject, with `400 validation_failed`, any `siteAddress` that's blank or doesn't end in 6 digits, via `@NotBlank`/`@Pattern(regexp = "^.*\\d{6}$")` plus a `@Valid` controller parameter — see `SPEC-booking-delivery-return-api.md` §9 1.2.0 and `SPEC-rental-plan-quote.md` §9 1.3.0. That means `sitePostalCode`'s substring formula, which always assumed the trailing 6 characters were digits but never enforced it, now actually gets a guaranteed-valid input on rows created or updated through those two DTOs. It's still not guaranteed for rows written any other way — `data.sql` seed inserts and any future direct-entity/`JpaRepository.save()` write path bypass `@Valid` entirely, since Bean Validation on JPA entity persist isn't enabled here (no `@NotBlank`/`@Pattern` on the entity fields themselves, only on the DTOs).
...
| `BookingItemRepository` | `BookingItem` | `List<BookingItem> findByBookingId(Long bookingId)` · `List<BookingItem> findByAssetId(Long assetId)` · `Set<Long> findAssetIdsWithOverlappingBooking(...)` (`@Query`) · `List<BookingItem> findByBookingStatusIn(Collection<BookingStatus> activeStatuses)` (`@Query`, `JOIN FETCH bi.booking`) — added on branch `hr-40-equipment-utilization-tracker` for `MonthlyUtilizationService` |
...
Three exceptions as of the utilization feature: `AssetRepository.findAllWithCategory()`, `BookingItemRepository.findAssetIdsWithOverlappingBooking(...)`, and `BookingItemRepository.findByBookingStatusIn(...)` all use `@Query`; the first returns `Set<Long>`, not entities, and the third uses `JOIN FETCH` to eagerly load each `BookingItem`'s `booking` (needed since `MonthlyUtilizationService` reads `booking.getStartDate()`/`getEndDate()` outside the repository call). Every other method still returns a full-entity `List<T>` (or `Optional<T>` / raw `T` for single lookups), with no `Pageable`/`Sort` variant or projection. None are `@Transactional` beyond Spring Data's defaults — transaction boundaries for the equipment feature's reads live in `AssetService`, not the repository layer.
...
| 1.6.0 | 2026-08-13 | **HR-116.** Noted, under §5.5/§5.7, that `siteAddress` remains an unconstrained nullable column at the DB/entity level, but `CreateBookingRequest`/`BookingUpdateRequest`/`RentalPlanCreateRequest` now enforce (DTO-layer `@NotBlank`/`@Pattern`, `@Valid` on the owning controller methods) that any value submitted through those three routes is non-blank and ends with a 6-digit postal code — giving `Booking.sitePostalCode`'s pre-existing `@Formula` substring a guaranteed-valid input on those paths, though not on seed data or any other direct write. No entity/repository/relationship content changed — documentation-only correction, per this doc's own convention. |
...
| 1.5.0 | 2026-08-11 | §8's `BookingItemRepository` row now lists `findByBookingStatusIn(...)`, added on branch `hr-40-equipment-utilization-tracker` to back `MonthlyUtilizationService`; the `@Query`-exceptions callout below the table updated from two methods to three, noting this one's `JOIN FETCH` is needed because the service reads `booking.getStartDate()`/`getEndDate()` outside the repository call. See [`SPEC-api-index.md`](./SPEC-api-index.md) §2.4 and [`CHANGES-monthly-utilization.md`](./CHANGES-monthly-utilization.md) for the route this backs. No entity/relationship facts changed. |
| 1.6.0 | 2026-08-12 | Added `Booking.returnNotes` (`return_notes`, `String(500)`, nullable) to §5.7 — new column introduced by HR-100 (`SPEC-booking-delivery-return-api.md` §5.2/Requirement 6). No other changes; the pre-existing `BookingStatus` enum drift (§5.7/§6.2 still show 5 values, missing HR-77's `PENDING_DEPOSIT`/`PENDING_CONFIRMED` split) and the `ddl-auto` drift noted in `SPEC-api-index.md` §6 remain unresolved — out of scope for this change. |
| 1.7.0 | 2026-08-13 | **HR-129.** §5.5 notes `RentalPlan.createdAt`/`updatedAt` are now actually written by application code (previously declared but always `null`) and exposed on `RentalPlanResponse` — see `SPEC-rental-plan-quote.md` REQ-2/REQ-3/REQ-6. No entity/repository/relationship content changed. (Picked `1.7.0` to land after both pre-existing `1.6.0` entries above rather than renumber this table's existing version collision — out of scope for this change.) |